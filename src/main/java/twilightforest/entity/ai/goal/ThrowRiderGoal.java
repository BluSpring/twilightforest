...
public class ThrowRiderGoal extends MeleeAttackGoal {
	...
	public boolean canUse() {
		return this.mob.getPassengers().isEmpty() &&
			...
			this.mob.getTarget().getAttachedOrCreate(TFDataAttachments.YETI_THROWING.get()).getThrowCooldown() <= 0 &&
			super.canUse();
	}
	...
	public void stop() {
		if (!this.mob.getPassengers().isEmpty()) {
			...
			if (rider instanceof Player player) {
				var attachment = player.getAttachedOrCreate(TFDataAttachments.YETI_THROWING.get());
