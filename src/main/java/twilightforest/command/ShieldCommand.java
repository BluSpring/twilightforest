...
public class ShieldCommand {
	...
	private int add(Entity e, int num, boolean temporary) {
		if (e instanceof LivingEntity living) {
			living.getAttachedOrCreate(TFDataAttachments.FORTIFICATION_SHIELDS.get()).addShields(living, num, temporary);
		}
	...
	private int set(Entity e, int num, boolean temporary) {
		if (e instanceof LivingEntity living) {
			living.getAttachedOrCreate(TFDataAttachments.FORTIFICATION_SHIELDS.get()).setShields(living, num, temporary);
		}
		return Command.SINGLE_SUCCESS;
	}
}
